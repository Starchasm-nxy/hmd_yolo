...
# def update_ux0(ux, prev_ux):
#     global prev_ux_global, prev_delta_ux, count2
#     if prev_ux_global is None:
#         prev_ux_global = ux
#         delta_ux = 0
#     else:
#         delta_ux = abs(ux - prev_ux_global)
        
#     if delta_ux > 100 and prev_delta_ux > 100:
#         count2 += 1
#         ux0 = prev_ux_global
#         prev_delta_ux = delta_ux
#     else:
#         count2 = 0
#         ux0 = ux
#         prev_ux_global = ux
    ...
    try:
        while not stop_event.is_set():
            ...
            results = model.predict(source=color_image, device='cpu', show=False, 
                                  stream=False, verbose=False, iou=0.45, conf=0.25)
            ...
            # 处理不同模式的逻辑
            if data_content == 'r':
                tong_list = [item for item in tong_list if item[2] < 424]
                if tong_list:
                    ...
                    if last_value is not None:
                        ...
                        print(f"[INFO] r模式未检测到桶，返回上一帧: {processed_list}")
                    else:
                        processed_list = [str(0)]
                ...
                if tong_list:
                    ...
                    if last_value is not None:
                        processed_list = last_value.split()
                        print(f"[INFO] l模式未检测到桶，返回上一帧: {processed_list}")
                    else:
                        processed_list = [str(0)]
                write_to_file(processed_list) 
            elif data_content == 'm':
                ...
                if tong_list:
                    ...
                    if last_value is not None:
                        processed_list = last_value.split()
                        print(f"[INFO] m模式未检测到桶，返回上一帧: {processed_list}")
                    else:
                        processed_list = [str(0)]
                write_to_file(processed_list)       
            ...
            if data_content == '1m':
                ...
                # if shen_list and not tong_list:
                #     if len(shen_list) == 3:
                #         if len(ss_X_list) < 30 and v == 0:
                #             sorted_shen_list = sorted(shen_list, key=lambda x: x[4])
                #             ss_X_list.append(sorted_shen_list[0][4])
                #             mm_X_list.append(sorted_shen_list[1][4])
                #         if len(ss_X_list) >= 30:
                #             v = 1
                #             ss_X_mean, mm_X_mean = qianshi_Y(ss_X_list, mm_X_list)
                #             ss_X_mean, mm_X_mean = worried_lm(ss_X_mean, mm_X_mean)
                #             new_list = [str(ss_X_mean), str(mm_X_mean)]
                #             count1 = qianshi_X(processed_list, count1, last_processed_list, new_list)
                #             write_to_file(processed_list)
                #     else:
                #         time.sleep(0.1)
                # elif not shen_list and not tong_list and v == 0:
                #     processed_list = [str(-2.6), str(0)]
                #     write_to_file(processed_list)
                # elif not shen_list and not tong_list and v == 1:
                #     processed_list = [str(ss_X_mean), str(mm_X_mean)]
                #     write_to_file(processed_list)
                ...
                #         if len(ss_X_list) < 30 and v == 0:
                #             sorted_shen_list = sorted(shen_list, key=lambda x: x[4])
                #             ss_X_list.append(sorted_shen_list[1][4])
                #             mm_X_list.append(sorted_shen_list[2][4])
                #         if len(ss_X_list) >= 30:
                #             v = 1
                #             ss_X_mean, mm_X_mean = qianshi_Y(ss_X_list, mm_X_list)
                #             ss_X_mean, mm_X_mean = worried_mr(ss_X_mean, mm_X_mean)
                #             new_list = [str(ss_X_mean), str(mm_X_mean)]
                #             count1 = qianshi_X(processed_list, count1, last_processed_list, new_list)
                #             write_to_file(processed_list)
                #     else:
                ...
                #             v = 1
                #             ss_X_mean, mm_X_mean = qianshi_Y(ss_X_list, mm_X_list)
                #             ss_X_mean, mm_X_mean = worried_lr(ss_X_mean, mm_X_mean)
                #             new_list = [str(ss_X_mean), str(mm_X_mean)]
                #             count1 = qianshi_X(processed_list, count1, last_processed_list, new_list)
                #             write_to_file(processed_list)
                #     else:
                #         time.sleep(0.1)
                # elif not shen_list and not tong_list and v == 0:
                #     processed_list = [str(-2.6), str(2.6)]
                #     write_to_file(processed_list)
                # elif not shen_list and not tong_list and v == 1:
                #     processed_list = [str(ss_X_mean), str(mm_X_mean)]
                #     write_to_file(processed_list)
