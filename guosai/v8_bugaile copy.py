...
# 所需权重
model = YOLO("/home/fu/weights/tongv3.pt")
...
def main_control(): 
    ...
    global t_test, t_detect, t_usb, camera
    last_c = None
    running = False
    camera = 0
    stop_event = None
    ...
    while True:
        if c != last_c:
            ...
            if c == 3:
                ...
                t_usb = threading.Thread(target=usb_near_detect, args=(model, shuchu_file_path, stop_event, start_event))
                t_usb.start()
                t_test = None
                t_detect = None
                camera = None
    ...
    while not stop_event.is_set():
        with open(shuru_file_path, 'r') as file2:
            ...
            if content != last_result:
                ...
                last_result = content
# USB相机检测（近距，原D435功能）
def usb_near_detect(model, shuchu_file_path, stop_event, start_event):
    cap = cv2.VideoCapture(cam_num)
    if not cap.isOpened():
        print("[ERROR] 无法打开USB摄像头")
        return
    start_event.wait()
    print("[DEBUG] usb_near_detect线程已启动")
    with open(shuchu_file_path, 'w') as file:
        file.write("0")
    result_str = "0"
    last_result = None
