...
def usb_detect(shuchu_file_path, stop_event, start_event, calib_file=None):
    ...
    # ---------- 视频录制设置 ----------
    output_dir = os.path.join(os.getcwd(), "recorded_videos")
    ...
    try:
        while not stop_event.is_set():
            ...
            if out is None or frame_count >= frames_per_segment:
                if out is not None:
                    out.release()
                    print(f"[INFO] 视频片段已保存，新片段开始")
                # 创建新视频文件
                timestamp = time.strftime("%Y%m%d_%H%M%S")
                filename = os.path.join(output_dir, f"segment_{timestamp}_{segment_idx:03d}.avi")
