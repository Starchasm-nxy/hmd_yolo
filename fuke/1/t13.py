...
def usb_detect(shuchu_file_path, stop_event, start_event, calib_file=None):
    ...
    # ---------- 视频录制设置 ----------
    base_dir = os.path.join(os.getcwd(), "recorded_videos")
    os.makedirs(base_dir, exist_ok=True)

    # 确定本次运行的子文件夹序号（自动递增，不覆盖历史）
    ...
    session_dir = os.path.join(base_dir, str(next_num))
    os.makedirs(session_dir, exist_ok=True)
    print(f"[INFO] 本次录制目录: {session_dir}")

    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    file_extension = '.avi'
    fps = USB_FPS
    frame_size = (USB_WIDTH, USB_HEIGHT)
    segment_duration = 10                     # 每段长度（秒）
    ...
    try:
        while not stop_event.is_set():
            ...
            if out is None or frame_count >= frames_per_segment:
                if out is not None:
                    out.release()
                    print(f"[INFO] 已保存片段 {segment_idx-1}")
                # 文件名：1.avi, 2.avi, ...
                filename = os.path.join(session_dir, f"{segment_idx}.avi")
                out = cv2.VideoWriter(filename, cv2.CAP_FFMPEG, fourcc, fps, frame_size)
                if not out.isOpened():
                    print(f"[ERROR] 无法创建视频文件 {filename}")
                    out = None
                else:
