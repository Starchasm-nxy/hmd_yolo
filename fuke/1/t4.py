...
# ==================== USB摄像头参数（参照程序二） ====================
USB_CAM_PATH = '/dev/video4'   # 可根据实际情况修改，如 0 或 '/dev/video2'
USB_WIDTH = 848
USB_HEIGHT = 480
USB_FPS = 30
BUFFER_SIZE = 2
CALIB_FILE = 'calib_resultC.npz' 
...
# ==================== 模型权重加载 ====================
model_1m = YOLO("/home/fu/权重/datong8.pt")
model_2m = YOLO("/home/fu/权重/datong8.pt")
...
def apply_mode_selection(data_content, tong_list):
    filter_map = {
        'r': lambda item: item[2] < 848,
        'm': lambda item: item[2] > 80 and item[2] < 768,
        'l': lambda item: item[2] > 0,
    }
...
# ==================== USB相机检测线程（原D435函数改造） ====================
def usb_detect(shuchu_file_path, stop_event, start_event):
    """核心检测线程：使用USB相机替换D435，其余逻辑不变"""
    global processed_list, last_processed_list, last_value

    if not os.path.exists(CALIB_FILE):
        print(f"[WARN] 标定文件 {CALIB_FILE} 不存在，将跳过畸变校正。")
        return
    try:
    ...
    if not cap.isOpened():
        ...
        start_event.wait()  # 避免立即退出，保持线程结构
        return
            ...
            for result in results:
                ...
                if boxes is not None:
                    for box in boxes:
                        ...
                        cv2.putText(canvas, str([ux, uy, '30cm']), (ux + 20, uy + 10), 0, 1,
...
def main_control():
    ...
    while not exit_event.is_set():
        if c != last_c:
            ...
            if c == 1:
                t_detect = threading.Thread(target=usb_detect, args=('gaozhi.txt', stop_event, start_event))
                t_detect.start()
                running = True
