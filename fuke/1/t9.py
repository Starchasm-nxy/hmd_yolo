...
USB_HEIGHT = 480
USB_FPS = 30
CALIB_FILE = 'calib_resultD.npz'   # 畸变校正文件
...
def draw_square(image, box, names, r):
    ...
    cv2.rectangle(image, (r[0], r[1]), (r[2], r[3]), (0, 0, 255), 2)
    cv2.putText(image, label, (r[0], r[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
    cv2.circle(image, (ux, uy), 5, (0, 0, 255), -1)
    return ux, uy
...
def usb_detect(shuchu_file_path, stop_event, start_event, calib_file=None):
    ...
    # 1. 设为手动曝光模式（值 1 或 0.75 均可尝试）
    # cap.set(cv2.CAP_PROP_AUTO_EXPOSURE, 1)   # 1 = 手动
    # # 2. 固定一个曝光绝对值（值越大越亮，需要根据环境调）
    # cap.set(cv2.CAP_PROP_EXPOSURE, 2048)      # 先试 156，再按实际亮度调整
    # # 固定自动曝光/增益/白平衡
    # cap.set(cv2.CAP_PROP_AUTO_EXPOSURE, 0.25)   # 手动曝光模式
    # cap.set(cv2.CAP_PROP_EXPOSURE, -6)          # 这个值需要你调试，先保持默认观察是否生效
    # cap.set(cv2.CAP_PROP_GAIN, 0)               # 增益
    # cap.set(cv2.CAP_PROP_AUTO_WB, 0)            # 自动白平衡
    cap.set(cv2.CAP_PROP_AUTO_WB, 1)   # 打开自动白平衡
    ...
    if calib_file and os.path.exists(calib_file):
        try:
            ...
            mtx = data['mtx']
            dist = data['dist']
            newcameramtx, roi = cv2.getOptimalNewCameraMatrix(mtx, dist, (USB_WIDTH, USB_HEIGHT), 1, (USB_WIDTH, USB_HEIGHT))
            ...
            for result in results:
                ...
                if boxes is not None:
                    for box in boxes:
                        ...
                        cv2.circle(canvas, (ux, uy), 4, (0, 0, 0), 1)
                        cv2.putText(canvas, str([ux, uy]), (ux + 10, uy + 5), 0, 1,
                                    [120, 120, 120], thickness=2, lineType=cv2.LINE_AA)
