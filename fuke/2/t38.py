...
import logging                            # 日志记录
from collections import deque             # 双端队列，用于FPS滑动窗口
...
# 模型文件路径
MODEL_LOCKED_PATH = "/home/fu/weights/tong_blue_v0.pt"     # 锁定（裁剪区域）时用的模型
MODEL_UNLOCKED_PATH = "/home/fu/weights/tongv3.pt"         # 未锁定（全图）时用的模型
...
# 模型推理参数
MODEL_LOCKED_CONF = 0.5               # 锁定模式下置信度阈值
MODEL_UNLOCKED_2M_IMGSZ = 640         # 未锁定2m模式推理尺寸
MODEL_UNLOCKED_1M_IMGSZ = 640         # 未锁定1m模式推理尺寸
MODEL_UNLOCKED_2M_CONF = 0.5          # 未锁定2m模式置信度阈值
MODEL_UNLOCKED_1M_CONF = 0.5          # 未锁定1m模式置信度阈值
MODEL_IOU = 0.45                      # NMS IoU阈值
...
def draw_lock_rect(canvas: np.ndarray, x: int, y: int, shw: int, shh: int,
    ...
    参数:
        ...
        color: 颜色元组 (B, G, R)
    """
    cv2.rectangle(canvas, (x - shw, y - shh), (x + shw, y + shh), color, 2)  # 搜索框
    cv2.circle(canvas, (x, y), 4, color, -1)                                # 中心十字准星点
        ...
        while not self._stop.is_set():
            try:
                if not self.clearfile:
                    logger.info("读取成功去除空白")
                    self.clearfile = not self.clearfile
                    self.connectfile = not self.connectfile
                with open(self.file_path, 'r') as f:
                    content = f.read().strip()          # 读取并去除空白
            except Exception:
                if not self.connectfile:
                    logger.info("读取失败等待重试")
...
class LockTracker:
    ...
    def _unlocked_with_dets(self, detections: List[Detection]) -> TrackResult:
        ...
        logger.info(f"锁定-最近桶：({ux},{uy}) 目标面积={w*h}")
        return TrackResult(TrackAction.DETECT, ux, uy, True)
...
class YOLODetector:
    """
    双模型YOLO检测器
    detect_full  → 使用 model_unlocked 对全图进行检测（根据模式调整conf/imgsz）
    detect_crop  → 使用 model_locked 对裁剪区域进行检测（动态imgsz）
    """

    def __init__(self) -> None:
        # 加载两个YOLO模型
        self.model_locked = YOLO(MODEL_LOCKED_PATH)       # 锁定模式下使用
        self.model_unlocked = YOLO(MODEL_UNLOCKED_PATH)   # 未锁定全图使用
        # 图像中心坐标（用于计算检测框到中心的距离）
        self._cx = D435_WIDTH // 2
        self._cy = D435_HEIGHT // 2

    def detect_full(self, frame: np.ndarray, mode: str) -> List[Detection]:
        """全图检测，根据模式选择置信度和imgsz"""
        # 根据模式确定置信度和推理尺寸
...
class DetectionPipeline:
    ...
    def run(self, stop_event: threading.Event,
        ...
        frame_count: int = 0             # 累计帧计数（用于跳帧逻辑）
        fps_times: deque = deque(maxlen=30) # 存储近期时间戳，用于计算FPS
        ...
        try:
            while not stop_event.is_set():
                ...
                if do_inference:
                    ...
                    self._process_frame(frame, canvas, mode)  # 核心处理
                    last_canvas = canvas.copy()           # 保存最新绘制结果
                else:
                    # 跳帧时复用上一次的画布，若无则直接用当前帧
    ...
    def _process_frame(self, frame: np.ndarray, canvas: np.ndarray,
        ...
        if was_locked:
            ...
            detections = self.detector.detect_crop(crop, csx1, csy1)  # 裁剪检测
            if detections:
                # 绘制黄色搜索框
