...
D435_WIDTH = 848
D435_HEIGHT = 480
MODEL_LOCKED_PATH = "/home/fu/weights/tongv6.pt"    # 锁定期间（裁剪推理）
...
LOCK_MAX_SEARCH_RADIUS = 240    # 搜索半径上限（像素，近目标）
...
def camera_detection_thread(stop_event):
    ...
    config = rs.config()
    config.enable_stream(rs.stream.depth, D435_WIDTH, D435_HEIGHT, rs.format.z16, 30)
