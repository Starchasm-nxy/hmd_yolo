...
# ==================== 跳帧控制 ====================
FRAME_SKIP_ENABLED = 1           # 0=每帧推理, 1=跳帧推理
...
LOCK_MAX_SEARCH_RADIUS = 270    # 搜索半径上限（像素，近目标）
    ...
    try:
        while not stop_event.is_set():
            ...
            if mode != last_mode:
                ...
                # 冷启动（从 '0' 进入 '1m' 或 '2m'）：集中清空所有历史
                if last_mode not in ['1m', '2m']:
                ...
                # 非冷启动的模式切换（1m<->2m）：只重置 lock 状态，保留 last_value
                if last_mode in ['1m', '2m'] and mode in ['1m', '2m']:
                    state.lock_target = None
                    state.lock_miss_count = 0
                    state.lock_frame_count = 0
                    ...
                    if state.lock_target is not None:
                        ...
                        if best_match is not None:
                            ...
                            if state.lock_miss_count > LOCK_MAX_MISS:
                                ...
                                processed_list = [str(2), str(lock_ox), str(lock_oy)]
                        ...
                        if state.lock_miss_count > LOCK_MAX_MISS:
                            ...
                            if state.last_value is not None:
                                if state.last_value == '0':
                                    ...
                                    processed_list = ['2'] + state.last_value.split()
                            else:
