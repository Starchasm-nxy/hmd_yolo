...
class LockTracker:
    ...
    def get_lock_target(self) -> Optional[Tuple[int, int, int, int]]:
        """线程安全地获取当前锁定目标"""
        with self._lock:
...
class DetectionPipeline:
    ...
    def _process_frame(self, frame: np.ndarray, canvas: np.ndarray,
        ...
        if was_locked:
            target = self.tracker.get_lock_target()
            if target is None:
...
class USBCameraSource:
    ...
    def start(self) -> None:
        ...
        if not self._cap.isOpened():
            raise RuntimeError(f"无法打开USB摄像头: {self.cam_path}")
            ...
            if clustered_once:
                ...
                if last_image is not None:
                    if not self.display_queue.full():
                        self.display_queue.put((last_image, DISPLAY_WIN_NAME_USB))
                time.sleep(0.1)
                continue
            ...
            if keydoor:
                ...
                if len(camera_centers) >= 3:
                    ...
                    if last_result != "0 0 0 0":
                        ...
                        last_result = "0 0 0 0"
                    clustered_once = True
                    final_result_str = "0 0 0 0"

                last_cluster_time = current_time
                last_image = image.copy() if image is not None else None
...
class PipelineManager:
    ...
    def _start_usb_pipeline(self, cmd: Command) -> None:
        ...
        if not os.path.exists(MODEL_USB_PATH):
            logger.error(f"{RED}USB模型文件不存在: {MODEL_USB_PATH}{RESET}")
            return
        ...
        try:
            ...
            logger.error(f"{RED}USB pipeline 启动失败: {e}{RESET}")
...
def main() -> None:
    ...
    # 预加载 USB 标定数据（避免显示循环中每帧重复加载）
    usb_calib_mtx = None
    try:
        calib_data = np.load(CALIB_FILE)
        usb_calib_mtx = calib_data['mtx']
        logger.info("USB标定文件已加载")
    except Exception as e:
        logger.warning(f"USB标定文件加载失败: {e}，四角坐标标注将不可用")
    ...
    try:
        while True:
            ...
            if win_name == DISPLAY_WIN_NAME_USB and usb_calib_mtx is not None:
                h, w = frame.shape[:2]
                corners = [(0, 0), (w-1, 0), (0, h-1), (w-1, h-1)]
                corner_colors = [(0, 0, 255), (0, 255, 0), (255, 0, 0), (0, 255, 255)]
                for i, (cx, cy) in enumerate(corners):
                    X, Y = pixel_to_camera(cx, cy, usb_calib_mtx, w, h)
