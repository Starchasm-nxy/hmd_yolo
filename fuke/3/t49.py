...
# ---- 窗口尺寸 ----
DISPLAY_WIN_W = 1300                               # 显示窗口宽度
DISPLAY_WIN_H = 900                                # 显示窗口高度
...
class YOLODetector:
    ...
    def detect_full(self, frame: np.ndarray, mode: str) -> List[Detection]:
        ...
        results = self.model_full.predict(
            source=frame, device=MODEL_DEVICE, show=False, stream=False, verbose=False, iou=iou, conf=conf, imgsz=imgsz)
        t1 = time.time()
    ...
    def detect_crop(self, crop: np.ndarray, ox: int, oy: int,
        ...
        results = self.model_crop.predict(
            source=crop, device=MODEL_DEVICE, show=False, stream=False, verbose=False, iou=iou, conf=conf, imgsz=imgsz)
        t1 = time.time()
        infer_time = int((t1 - t0) * 1000)
    ...
    def run(self, stop_event: threading.Event,
        ...
        while not stop_event.is_set():
            ...
            results = self._model.predict(
                source=frame, device=MODEL_DEVICE, show=False, stream=False, verbose=False, iou=USB_IOU, conf=USB_CONF, imgsz = USB_IMGSZ)
