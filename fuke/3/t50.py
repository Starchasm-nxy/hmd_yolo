...
# ---- 超椭圆绘制参数 ----
SUPERELLIPSE_P_LOCK = 2.7                      # draw_lock_rect / LockTracker 超椭圆指数
...
class ModeState:
    """线程安全共享模式状态，替代全局变量 c/b/d 和 start_time"""
...
class YOLODetector:
    ...
    def detect_full(self, frame: np.ndarray, mode: str) -> List[Detection]:
        ...
        t0 = time.time()
        results = self.model_full.predict(
    ...
    def detect_crop(self, crop: np.ndarray, ox: int, oy: int,
        ...
        t0 = time.time()
        results = self.model_crop.predict(
...
# ======================================================================
# 10.5 DBSCANClusterer — DBSCAN 聚类 + 可视化
# ======================================================================
...
# ======================================================================
# 11.5 FileModeMonitor — data.txt 文件模式监控器
# ======================================================================
...
# ======================================================================
# 11.6 USBDetectionPipeline — USB 检测管道
# ======================================================================
    ...
    def run(self, stop_event: threading.Event,
        ...
        while not stop_event.is_set():
            ...
            results = self._model.predict(
                source=frame, device=MODEL_DEVICE, show=False,
                stream=False, verbose=False, iou=USB_IOU, conf=USB_CONF)
...
# ======================================================================
# 11.7 D435DetectionPipeline — D435 检测管道
# ======================================================================
...
# ======================================================================
# 11.8 PipelineOrchestrator — 管道生命周期管理器
# ======================================================================

class PipelineOrchestrator:
    """根据 ModeState.c 变化启停 USB/D435 管道（替代 main_control）"""
