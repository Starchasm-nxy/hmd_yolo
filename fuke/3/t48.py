...
def draw_square(image: np.ndarray, box: Any, names: Dict[int, str],
                r: List[int], p: float = 2.5, num_points: int = 100) -> Tuple[int, int]:
    """
    以超椭圆绘制检测框、标签和中心点。
    返回: (ux, uy) 框的中心点坐标
    """
    ...
    # 超椭圆：|cos(t)/shw|^p + |sin(t)/shh|^p = 1/r^p
    ...
    cv2.polylines(image, [pts], isClosed=True, color=(221, 185, 193), thickness=2)
    cv2.putText(image, label, (ux - 30, uy - shh - 10),
...
def draw_lock_rect(canvas: np.ndarray, x: int, y: int, shw: int, shh: int,
                   color: Tuple[int, int, int], p: float = 2.3, num_points: int = 100) -> None:
    """
        ...
        if not frame_queue.empty():
            ...
            if keydoor:
                ...
                if len(camera_centers) >= 3:
                    ...
                    if (-4 <= center1[0] <= 4 and -4 <= center2[0] <= 4
                            and -2.5 <= center1[1] <= 2.5
                            and -2.5 <= center2[1] <= 2.5):
