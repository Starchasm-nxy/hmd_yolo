...
max_time = 2.0                                   # 最大等待时间（秒），稳定后收集上限
files_to_clear = ['data.txt', 'gaozhi.txt']      # 启动时需要清空的文件列表

# ---- USB 相机配置 ----
files_for_pixel = 'calib_resultC.npz'            # 畸变校正文件（根据相机改变）
...
SHOW_COLUMN_LINES = True                            # 是否在 USB 窗口画共列竖虚线
...
CLUSTER_TIMEOUT = 5.0                             # USB 聚类超时（秒）
CLUSTER_STABILIZE_DELAY = 1.0                     # 聚类开始前稳定延时（秒）
CLUSTER_RETRY_INTERVAL = 0.7                      # 聚类失败后重试间隔（秒）
CLUSTER_TIME_WINDOW = 2.0                         # 时间窗（秒），旧点不参与聚类
CLUSTER_DECAY_TAU = 1.5                           # 衰减时间常数（秒），越小越信新点
...
class DBSCANClusterer:
    ...
    def cluster(self, image: np.ndarray,
        ...
        """运行 DBSCAN（时间窗过滤 + 衰减加权中心），返回聚类中心列表"""
        if len(points) == 0:
            logger.info("没有桶")
            return []
        ...
        # ---- Step 1: 时间窗过滤，只保留最近的点 ----
        recent = [(x, y, t) for x, y, t in points
                  if now - t <= CLUSTER_TIME_WINDOW]
        if len(recent) == 0:
            logger.info("时间窗内无有效点")
            return []

        # ---- Step 2: DBSCAN 聚类（只用 xy 坐标） ----
        xy = np.array([(x, y) for x, y, _ in recent], dtype=np.float64)
        timestamps = np.array([t for _, _, t in recent])
        db = DBSCAN(eps=self._eps, min_samples=self._min_samples)
        ...
        for i in range(n_clusters):
            ...
            # 指数衰减权重
            weights = np.exp(-(now - cluster_ts) / CLUSTER_DECAY_TAU)
            total_w = weights.sum()
            cx = float((cluster_xy[:, 0] * weights).sum() / total_w)
            cy = float((cluster_xy[:, 1] * weights).sum() / total_w)
            cluster_center = np.array([cx, cy])
            cluster_centers.append(cluster_center)

            # ---- 可视化 ----
            if image is not None:
                for pt in cluster_xy:
        ...
        cv2.imwrite(f"聚类结果图/cluster_{int(time.time())}_{attempt}.jpg", image)
...
class USBDetectionPipeline:
    ...
    def _resolve_barrel_order(
            ...
            threshold: float) -> Tuple[List[Tuple[float, float]],
                                        List[List[Tuple[float, float]]]]:
        """
        根据几何布局将 3 个桶排成 [l目标, m目标, r目标]。
        规则：检测共列 → m=后桶(Y大), l/r=前桶或独列桶。
        返回 (ordered, groups):
          ordered: 长度为 3 的列表，索引 0/1/2 对应 l/m/r
          groups:  按 X 分组的列表，可用于可视化
        """
        # 1. 按 X 分组
    ...
    @staticmethod
    def _draw_column_lines(image: np.ndarray,
                           groups: List[List[Tuple[float, float]]],
                           mtx: np.ndarray, w: int, h: int) -> None:
        """在图像上画竖虚线表示共列分组，同组同色"""
        fx, cx = mtx[0, 0], w / 2
        colors = [(0, 255, 0), (0, 255, 255), (255, 0, 255)]  # 绿 黄 紫

        for gi, g in enumerate(groups):
            color = colors[gi % 3]
            for (X_m, _) in g:
                px = int(cx + X_m * fx / Z)          # 米→像素 X
                if px < 0 or px >= w:
                    continue
                for py in range(0, h, 15):           # 每 15px 一段虚线
                    cv2.line(image, (px, py), (px, min(py + 10, h - 1)), color, 1)
    ...
    def run(self, stop_event: threading.Event,
        ...
        points: List[Tuple[int, int, float]] = []  # (ux, uy, timestamp)
        # 稳定延时：前 CLUSTER_STABILIZE_DELAY 秒不攒点，但 YOLO 照跑、画面照显
        start_collect_time = time.time() + CLUSTER_STABILIZE_DELAY
        start_cluster_time = time.time()
        clustered_once = False
        final_result_str: Optional[str] = None
        first_triggered = False          # 首次聚类是否已触发
        last_retry_time: float = 0.0     # 上次重试的时间戳
        cluster_attempt = 0              # 聚类次数计数器
        # 共列竖虚线
        _column_groups: Optional[List[List[Tuple[float, float]]]] = None
        _column_lines_until: float = 0.0

        while not stop_event.is_set():
            if (not clustered_once
                    and time.time() - start_cluster_time > CLUSTER_TIMEOUT):
                for _ in range(5):    
                    yellow_log(f"聚类超时({CLUSTER_TIMEOUT}s)，输出 0 0 0 0 并进入 0 模式")
                self._writer.write_usb("0 0 0 0")
                self._mode_state.set_usb_used()
                self._mode_state.set(0)
                self._mode_state.set_pause_start(time.time())  # 重置计时器，防误超时
                break
            ...
            for result in results:
                ...
                for box in boxes:
                    ...
                    cv2.putText(image, f"({x:.2f}m, {y:.2f}m)",
                                ...
                                1, (240, 240, 240), 3)
                    # 稳定延时内不攒点（飞机还在晃）
                    if time.time() >= start_collect_time:
                        points.append((ux, uy, time.time()))

            current_time = time.time()

            # ---- 聚类触发：首次用点数/时间条件，失败后每秒重试 ----
            do_cluster = False
            if not first_triggered:
                # 首次触发：攒够点数 OR 超过最大等待时间
                if len(points) >= min_samples:
                    do_cluster = True
                elif current_time - start_collect_time >= max_time and len(points) > 0:
                    do_cluster = True
                if do_cluster:
                    first_triggered = True
                    last_retry_time = current_time
            else:
                # 失败重试：每隔 CLUSTER_RETRY_INTERVAL 秒再聚一次
                if current_time - last_retry_time >= CLUSTER_RETRY_INTERVAL:
                    do_cluster = True
                    last_retry_time = current_time

            if do_cluster:
                cluster_attempt += 1
                centers = self._clusterer.cluster(image, points, cluster_attempt)
                camera_centers = []
                ...
                if len(camera_centers) == 3:
                    ...
                    ordered, groups = self._resolve_barrel_order(
                        camera_centers, COLUMN_X_THRESHOLD)
                    # 记录分组用于窗口竖虚线显示（0.5s 后消失）
                    _column_groups = groups
                    _column_lines_until = time.time() + 0.5
                    ...
                    for _ in range(5):
                        yellow_log(f"聚类第 {cluster_attempt} 次 → 3 簇，成功 ✓")
                    clustered_once = True
                    final_result_str = result_str
                    self._mode_state.set_usb_used()
                    self._mode_state.set(0)  # 通知 Orchestrator 切换 D435
                    self._mode_state.set_pause_start(time.time())
                else:
                    for _ in range(5):
                        yellow_log(f"聚类第 {cluster_attempt} 次 → {len(camera_centers)} 簇，失败")
            ...
            if 'image' in locals():
                # 画共列竖虚线（每次聚类后显示 0.5s）
                if SHOW_COLUMN_LINES and _column_groups is not None:
                    if time.time() < _column_lines_until and mtx is not None:
                        self._draw_column_lines(image, _column_groups, mtx, w, h)
                    else:
                        _column_groups = None  # 过期清除
...
class D435DetectionPipeline:
    ...
    def run(self, stop_event: threading.Event,
        ...
        # ---- 预热预览：显示原始帧，等 b=1/2 且模型就绪 ----
        last_seq = 0
        while not stop_event.is_set():
            got, frame, last_seq = self._camera_src.get_frame(last_seq)
            if got and not self._display_queue.full():
                self._display_queue.put((frame, "d435"))
            b = self._mode_state.get_b()
            if b in (1, 2) and model_ready.is_set():
                break
            time.sleep(POLL_SLEEP_FAST)

        model_ready.wait()  # 双保险
        loader.join()
...
class PipelineOrchestrator:
    ...
    def __init__(self, mode_state: 'ModeState', writer: OutputWriter,
        ...
        self._current_stop_event: Optional[threading.Event] = None  # 当前管线的 stop_event
    ...
    def run(self, start_event: threading.Event) -> None:
        ...
        self._usb_detect_thread = threading.Thread(
            ...
            args=(self._current_stop_event, start_event))
        self._usb_detect_thread.start()
        self._running = True
        yellow_log("USB 相机已开启（预热预览中，等待指令）")
        ...
        while True:
            ...
            if c == 3:
                # ---- USB 检测模式 ----
                # USB 管线已在 Phase 0 创建并预览中，start_event 已触发
                # 管线正在自动切换到检测状态，无需额外操作
                self._d435_thread = None
                self._d435_camera = None
                yellow_log("USB 管线已进入检测模式")
                ...
                # ---- D435 暂停 / USB 完成 → 预热 D435 ----
                if not self._mode_state.get_usb_used():
                    # USB 还没用过，无视（set_from_command 已拦截）
                    pass
                elif self._d435_thread is not None and self._d435_thread.is_alive():
                    # D435 正在运行 → 暂停，管线内部处理 b=0，不停止相机
                    pass
                elif self._d435_camera is not None:
                    # D435 相机已开但管线未启动（极端情况），跳过
                    pass
                else:
                    # USB 完成 → 停止 USB，立刻开 D435 预热
                    self._stop_pipeline()
                    self._d435_camera = CameraSource()
                    self._d435_camera.start()
                    self._current_stop_event = threading.Event()
                    ...
                    self._d435_thread = threading.Thread(
                        ...
                        args=(self._current_stop_event, start_event))
                    self._d435_thread.start()
                    yellow_log("D435 相机已开启（预热预览中，等待 1m/2m）")
                    self._running = True
